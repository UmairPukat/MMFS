...
{
    public class FirstTimeLoginMapping
    {
        public FirstTimeLoginMapping(EntityTypeBuilder<FirstTimeLogin> entityTypeBuilder)
        {
            entityTypeBuilder.HasKey(m => m.Id);
            entityTypeBuilder.Property(m => m.Email).HasColumnName("Email").HasMaxLength(50).IsRequired(true);
